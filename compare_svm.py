...
# Create your models
model_svm = Sskl.LogisticRegression(C=0.01,penalty='l1',solver='liblinear',max_iter=300)
model_svm_unused = SVC(C=10,gamma=0.001,kernel='rbf')  # SVM model

# Fit the models
model_svm.fit(x_train, y_train)
print("SVM Fitted")

model_svm_unused.fit(x_train_unused, y_train)
print("SVM Unused Fitted")

# Make predictions
pred_svm = model_svm.predict(x_test)
print("SVM Predicted")

pred_rf = model_svm_unused.predict(x_test_unused)
print("SVM Unused Predicted")

# Compute the accuracy scores
acc_svm = model_svm.score(x_test, y_test)
acc_unused = model_svm_unused.score(x_test_unused, y_test)
...
df_svm = pd.DataFrame(tb_svm, columns=['Correct SVM Unused', 'Incorrect SVM Unused'], index=['Correct SVM', 'Incorrect SVM'])
